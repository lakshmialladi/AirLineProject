...
public class FlightServiceImp implements FlightService
{
	...
	public List<FlightDTO> getAllFlights()
	{
		List<FlightDTO> list = flightRepository.findAll().stream().map(flightDb -> FlightDTO.builder().destination(flightDb.getDestination())
														...
														.fares(FareDTO.builder().currency(flightDb.getFare().getCurrency())
																.amount(flightDb.getFare().getAmount()).build())
														.flightInfos(FlightInfoDTO.builder().flightNumber(flightDb.getFlightInfo().getFlightNumber())
																.flightTime(flightDb.getFlightInfo().getFlightTime())
																.flightType(flightDb.getFlightInfo().getFlightType()).build())
														.inventories(InventoryDTO.builder().count(flightDb.getInventory().getCount()).build())
													.build()).toList();
		return list;
	}
	...
	{
		return getAllFlights().stream().filter(flight -> flight.getFlightInfos().getAirLineInfos().getAirLineName().equals(flightDTO.getFlightInfos().getAirLineInfos().getAirLineName())
													  && flight.getFlightNumber().equals(flightDTO.getFlightNumber())
													  && flight.getFlightDate().equals(flightDTO.getFlightDate())
